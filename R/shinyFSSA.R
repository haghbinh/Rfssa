...
server.fssa <- function(input, output, clientData, session) {
  ...
  output$res.plot <- renderPlot({
    ...
    if (input$desc %in% c("fssa.reconst", "ssa.reconst") && !is.null(input$rec.type)) if (input$rec.type %in% c("heatmap", "line", "3Dline", "3Dsurface")) {
      return()
    }
    if (input$f.choice == "server") {
  ...
  output$res.ly <- renderPlotly({
    ...
    if (input$desc %in% c("fssa.reconst", "ssa.reconst")) if (!input$rec.type %in% c("heatmap", "line", "3Dline", "3Dsurface")) {
      return()
    }
    sr <- run_ssa()
    input.g <- eval(parse(text = paste0("list(", input$g, ")")))
